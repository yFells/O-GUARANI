...
"""
    ...
    def _gerar_resposta_semantica(self, pergunta: str, chunks: List[Dict]) -> str:
        ...
        self._log("🔬 Iniciando nova abordagem: Resumo de Personagem + Contexto da Pergunta.")

        # 1. IDENTIFICAR PERSONAGEM NA PERGUNTA USANDO SPACY
        pergunta_doc = self.nlp(pergunta)
        personagens_pergunta = {ent.text for ent in pergunta_doc.ents if ent.label_ == "PER"}
        personagem_principal = list(personagens_pergunta)[0] if personagens_pergunta else None

        contexto_personagem = ""
        if personagem_principal:
            self._log(f"👤 Personagem identificado na pergunta: {personagem_principal}")

            # 2. CRIAR UM "RESUMO" DO PERSONAGEM
            # Para isso, fazemos uma busca semântica focada apenas no nome do personagem.
            # O chunk com a maior similaridade ao nome do personagem provavelmente o descreve.
            similaridades_personagem = self.calcular_similaridade_semantica(personagem_principal)
            
            if any(similaridades_personagem):
                best_chunk_idx = np.argmax(similaridades_personagem)
                contexto_personagem = self.text_chunks[best_chunk_idx]
                self._log(f"📚 Contexto do personagem encontrado (chunk {best_chunk_idx}): {contexto_personagem[:300]}...")

            # 3. ENRIQUECER O CONTEXTO DA PERGUNTA
            # Combinamos o resumo do personagem com a pergunta original para uma busca mais precisa.
            if contexto_personagem:
                contexto_de_busca = f"{contexto_personagem}\n\nCom base nisso, responda: {pergunta}"
                self._log("🧠 Contexto de busca enriquecido criado.")
            else:
                contexto_de_busca = pergunta
                self._log("⚠️ Nenhum personagem encontrado, usando pergunta original para a busca.")

            # 4. BUSCA SEMÂNTICA COM CONTEXTO ENRIQUECIDO
            # Agora, usamos este novo contexto para calcular as similaridades finais.
            # Isso direciona a busca para chunks que correspondem à SITUAÇÃO da pergunta.
            similaridades_finais = self.calcular_similaridade_semantica(contexto_de_busca)

            # Criar e ordenar os resultados com base nas novas similaridades
            chunk_results_finais = []
            for i, sim in enumerate(similaridades_finais):
                chunk_results_finais.append({
                    'chunk_id': i,
                    'chunk': self.text_chunks[i],
                    'similarity': float(sim)
                })
            chunk_results_finais.sort(key=lambda x: x['similarity'], reverse=True)

            # 5. GERAR A RESPOSTA FINAL
            best_chunk = chunk_results_finais[0] if chunk_results_finais else None

            if best_chunk and best_chunk['similarity'] >= self.similarity_threshold:
                self._log(f"🏆 Melhor chunk encontrado com contexto (ID {best_chunk['chunk_id']}, Similaridade: {best_chunk['similarity']:.3f})")
                
                # Extrair a melhor sentença do chunk mais relevante (opcional, mas melhora a precisão)
                # (Pode-se reusar a lógica de re-ranking de sentenças do código original aqui, 
                # mas usando o `contexto_de_busca` como a "pergunta")
                
                intro = "Com base na análise contextual da obra, a informação mais relevante é:\n\n"
                main_content = best_chunk['chunk']
                if len(main_content) > 700:
                    main_content = main_content[:700] + "..."
                ...
                max_sim = best_chunk['similarity'] if best_chunk else 0.0
                self._log(f"⚠️ Nenhum chunk relevante encontrado com o contexto enriquecido (Max Sim: {max_sim:.3f}).")
                return self._resposta_nao_encontrada_semantica(pergunta, max_sim)
